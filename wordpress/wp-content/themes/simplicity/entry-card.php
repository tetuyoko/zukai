  ...
  <div class="entry-thumb">
    <?php if ( is_list_style_thumb_cards() ): //デフォルトのサムネイルカード表示の場合?>
      <?php if ( has_post_thumbnail() ): // サムネイルを持っているとき ?>
        <a href="<?php the_permalink(); ?>" class="entry-image entry-image-link"><?php the_post_thumbnail( 'thumb150', array('style' => 'width:150px;height:150px;', 'class' => 'entry-thumnail') ); ?></a>
      <?php else: // サムネイルを持っていない ?>
        <a href="<?php the_permalink(); ?>" class="entry-image entry-image-link"><img src="<?php echo get_template_directory_uri(); ?>/images/no-image.png" alt="NO IMAGE" title="NO IMAGE" style="width:150px;height:150px;" class="entry-thumnail no-image list-no-image" /></a>
      <?php endif; ?>
    <?php else: //大きなサムネイルカードの場合?>
      <?php if ( has_post_thumbnail() ): // サムネイルを持っているとき
        //サムネイル画像の縦横比を保存するかどうかで取得するサムネイルを変化
        $thumb = ( is_list_style_tile_thumb_cards_raw() ? 'thumb320_raw' : 'thumb320') ?>
        <a href="<?php the_permalink(); ?>" class="entry-image entry-image-link"><?php the_post_thumbnail($thumb , array('style' => 'width:320px;height:auto;', 'class' => 'entry-thumnail') ); ?></a>
      <?php else: // サムネイルを持っていないとき ?>
        <a href="<?php the_permalink(); ?>" class="entry-image entry-image-link"><img src="<?php echo get_template_directory_uri(); ?>/images/no-image-320.png" alt="NO IMAGE" title="NO IMAGE" style="width:320px;height:180px;" class="entry-thumnail no-image list-no-image" /></a>
      <?php endif; ?>
    <?php endif; ?>
  </div><!-- /.entry-thumb -->
  <div class="entry-content">

    <h2><a href="<?php the_permalink(); ?>" class="entry-title entry-title-link"><?php echo get_the_title(); ?></a></h2>
    <p class="post-meta">
